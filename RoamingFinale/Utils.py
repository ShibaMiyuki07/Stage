...
def insertion_database(day,donne):
    ...
    resultat = collection.find({'day' : day,'usage_type' : 'roaming'})
    count = 0
    for r in resultat:
        count += 1
    ...
    if count>0:
        ...
        for r in list_key:
            collection.update_one({'day' : day,'usage_type' : 'roaming'},{"$set" : {r : donne[r]}})
    else:
...
def get_all_mcc():
    connexion = mysql.connector.connect(user='ETL_USER',password='3tl_4ser',host='192.168.61.196',database='DM_RF')
    cursor = connexion.cursor() 
    query = "select distinct(mcc) as name from rf_mcc_mnc where mcc is not null"
    cursor.execute(query)
    all_site = []
    for(name) in cursor:
...
def get_all_network():
    connexion = mysql.connector.connect(user='ETL_USER',password='3tl_4ser',host='192.168.61.196',database='DM_RF')
    cursor = connexion.cursor() 
    query = "select distinct(network) as name from rf_mcc_mnc where network is not null"
    cursor.execute(query)
    all_site = []
    for(name) in cursor:
        all_site.append(name[0])
...
def getop_code():
    ...
    for(op_code) in cursor:
        liste_op.append(op_code[0])
    liste_op.append('null')
    return liste_op