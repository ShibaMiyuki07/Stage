...
def insertion_day(day,date):
    ...
    data = []
    for (nb) in cursor:
        data.append({ 'day' : date,'usage_type' : 'e-rc','type_aggregation' : 'day','nb' : nb[0]})
...
def insertion_site_name(day,date):
    ...
    for (site_name,nb) in cursor:
        data.append({ 'day' : date,'usage_type' : 'e-rc','type_aggregation' : 'site_name','site_name' : site_name,'nb' : nb})
