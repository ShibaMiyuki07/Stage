...
def insertion_database(day,donne):
    ...
    resultat = collection.find({'day' : day,'usage_type' : 'usage'})
    count = 0
    for r in resultat:
        count += 1
    ...
    if count>0:
        ...
        for r in list_key:
            collection.update_one({'day' : day,'usage_type' : 'usage'},{"$set" : {r : donne[r]}})
    else:
...
def getall_op_code():
    connexion = mysql.connector.connect(user='ETL_USER',password='3tl_4ser',host='192.168.61.196',database='DM_RF')
    cursor = connexion.cursor() 
    query = "select distinct(name) as name from rf_operator where name is not null"
    cursor.execute(query)
    all_site = []
    for(name) in cursor:
